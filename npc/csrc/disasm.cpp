...
void init_disasm() {
  ...
  cs_disasm_dl = reinterpret_cast<size_t (*)(csh handle, const uint8_t *code,
      size_t code_size, uint64_t address, size_t count, cs_insn **insn)>(
      dlsym(dl_handle, "cs_disasm"));
  assert(cs_disasm_dl);

  cs_free_dl = reinterpret_cast<void (*)(cs_insn *insn, size_t count)>(
...
void disassemble(char *str, int size, uint64_t pc, uint8_t *code, int nbyte) {
  ...
  printf("DEBUG: count = %lu, insn[0] = %s\n", count, insn->mnemonic);
  assert(count == 1);
