...
class WBU extends Module with HasYsyxModuleName {
    ...
    io.rf.wb_we := io.in.bits.wr_reg && allow_side_effect
    io.rf.wb_addr := io.in.bits.rd

    io.csr.mret := io.in.bits.mret && allow_side_effect
    io.csr.wb_ex := false.B && valid
    io.csr.wb_cause := 0.U(32.W)
    io.csr.wb_pc := 0.U(32.W)

    io.csr.csrReq.re := io.in.bits.csrReq.re && allow_side_effect
    io.csr.csrReq.we := io.in.bits.csrReq.we && allow_side_effect
    ...
    when(has_exception) {
        wb_cause := io.in.bits.exception_code
    }.elsewhen(io.in.bits.ecall) {
        switch(io.csr.priv) {
    ...
    when(has_exception || io.in.bits.ecall) {
        io.csr.wb_ex := valid
    ...
    halt.halt_valid := (io.in.bits.ebreak || io.in.bits.inv) && allow_side_effect
